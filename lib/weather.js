...
function getRainData(body, second) {
	// new regex compatible with updated data (which contains a block of commented-out JSON)
	let match = body.match(/var\s*hccompact_data_rain\s*=\s*\[[\s\S]*?^\s*(?!\/\/)\s*name:\s*'(Regen|Schnee)'[\s\S]*?^\s*(?!\/\/)\s*data:\s*(\[[\s\S]*?\])[\s\S]*?^\s*(?!\/\/)\s*name:\s*'(Regen|Schnee)'[\s\S]*?^\s*(?!\/\/)\s*data:\s*(\[[\s\S]*?\])[\s\S]*?\]\s*;/m);
		...
		if(!error && response && (response.statusCode === 200 || response.statusCode === 204)) {
			...
			baseRequest.get({
				...
				if(!error && response && (response.statusCode === 200 || response.statusCode === 204)) {
					...
					adapter.log.warn('Error getting forecast data from url ' + fc_url_2 + ': ' + error + ', response: ' + response + ', status: ' + (response ? response.statusCode : 'null'));
				}
				normalizeRainData();
			...
			adapter.log.warn('Error getting forecast data from url ' + fc_url + ': ' + error + ', response: ' + response + ', status: ' + (response ? response.statusCode : 'null'));
			normalizeRainData();
			normalizeRainData('secondary');
			onForecastDataReady();
			callback && callback();
...
function getCityId(callback) {
	// the kachelmannwetter API to locate a city based on coordinates now requires a paid subscription, use override city information if provided
	if(adapter.config.forecast_city_id && Number.isInteger(adapter.config.forecast_city_id)) {
		adapter.log.info('Using fallback city id for kachelmannwetter');
		city_id = adapter.config.forecast_city_id;
		city_name = adapter.config.forecast_city_name ? adapter.config.forecast_city_name : 'unknown';
		forecast_web_url = 'https://kachelmannwetter.com/de/vorhersage/'+ city_id +'-' + city_name.toLowerCase() +'/kompakt/deu-hd';
