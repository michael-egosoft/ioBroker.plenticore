...
//OLD2   let match = body.match(/var\s*hccompact_data_rain\s*=\s*\[[\s\S]*?name:\s*'(Regen|Schnee)'[\s\S]*?data:\s*(\[[\s\S]*?\]\s*)\s*,\s*[\s\S]*?\}\s*\}\s*(?:,\s*\{[\s\S]*?name:\s*'(Regen|Schnee)'[\s\S]*?data:\s*(\[[\s0\S]*?\]\s*)\s*,[\s\S]*?\s*\})\s*\]\s*;/);
...
function getCityId(callback) {
	// kachelmannwetter changed the API and requires a login now
	adapter.log.info('Using fallback city id for coordinates');
	city_id = 2945932;
	city_name = 'Boscheln';
	forecast_web_url = 'https://kachelmannwetter.com/de/vorhersage/2945932-boscheln/kompakt/deu-hd'
